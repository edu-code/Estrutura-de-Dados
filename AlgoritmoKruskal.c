...
// Sobre o algoritmo:
// O algoritmo de Kruskal é um algoritmo de grafos utilizado para encontrar a Árvore Geradora Mínima (AGM)
// em um grafo ponderado e conexo. Uma Árvore Geradora Mínima é uma subárvore que contém todos os vértices
// do grafo original e forma uma árvore acíclica, minimizando o peso total das arestas. 
// No código apresentado, o grafo representa uma rede de comunicação com pesos nas arestas indicando os custos
// de conexão entre os vértices. A AGM encontrada pelo algoritmo de Kruskal seria a estrutura para conectar 
// todos os pontos com custo total mínimo.
// Utilização:
// O algoritmo de Kruskal é amplamente utilizado em situações em que é necessário conectar um conjunto de pontos
//  com um conjunto mínimo de conexões (arestas) com pesos mínimos. Ele é uma escolha comum para problemas de 
//  design de redes e otimização de custos em engenharia de sistemas de comunicação e transporte.