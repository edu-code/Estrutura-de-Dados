...
// Funcionamento: Os registros armazenados em uma tabela são endereçados a partir de uma transformação
//  aritmética sobre a chave de pesquisa. Busca ter eficiência O(1) nas operações de busca, inserção e remoção. 
// Para isso, as inserções e remoções não devem provocar grandes variações na quantidade de registros armazenados.

// É constituído de duas etapas principais:
// 1. Computar o valor da função de dispersão, a qual transforma a chave de pesquisa em um
// endereço da tabela.
// 2. Desenvolver um método para lidar com as colisões.

// Endereçamento: Quando o universo de chaves U é pequeno, podemos alocar uma tabela com uma posição
// para cada chave, ou seja, |T| = |U|.
// • Então, cada posição da tabela, que pode ser implementada como um vetor, representa uma chave de U e 
// armazena um elemento ou um ponteiro para o elemento.

// Endereçamento Aberto: Em uma tabela de hash com endereçamento aberto, todos os elementos são armazenados
//  na própria tabela e sem o uso de listas encadeadas.
// O fator de carga não pode exceder o valor 1.
// • O espaço gasto com encadeamento é economizado e a colisão é tratada com a busca de uma posição vazia na
// própria tabela para inserção.
...
// – Colisão: Efeito que acontece quando duas chaves
// são mapeadas para a mesma posição na tabela.
